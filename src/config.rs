...
pub fn load_from_path(path: &Path) -> Result<RuntimeConfig, CompositorError> {
    ...
    let values = if looks_like_hyprland_style(&content) {
        parse_hyprland_style(&content)?
    } else {
        load_lua_values(path)?
    };
...
fn looks_like_hyprland_style(content: &str) -> bool {
    for raw_line in content.lines() {
        ...
        if line.starts_with('$')
            || is_assignment_for(line, "bind")
            || is_assignment_for(line, "bindm")
            || is_assignment_for(line, "exec-once")
            || line.eq_ignore_ascii_case("general {")
            || line.eq_ignore_ascii_case("input {")
            || line.eq_ignore_ascii_case("decoration {")
            || line.eq_ignore_ascii_case("animations {")
            || line.eq_ignore_ascii_case("misc {")
            || is_assignment_for(line, "monitor")
            || is_assignment_for(line, "env")
        {
            return true;
        }
...
fn parse_hyprland_style(content: &str) -> Result<HashMap<String, String>, CompositorError> {
    ...
    let mut autostart_index: usize = 1;
    ...
    for raw_line in content.lines() {
        ...
        if block == Block::General {
            ...
            if let Some((raw_key, raw_value)) = line.split_once('=') {
                ...
                match key.as_str() {
                    "gaps_in" => {
                        values.insert("gaps.inner_horizontal".to_owned(), value.to_owned());
                        values.insert("gaps.inner_vertical".to_owned(), value.to_owned());
                    }
                    "gaps_out" => {
                        values.insert("gaps.outer_horizontal".to_owned(), value.to_owned());
                        values.insert("gaps.outer_vertical".to_owned(), value.to_owned());
                    }
                    "border_size" => {
                        values.insert("border_size".to_owned(), value.to_owned());
                    }
                    _ => {}
                }
            }
        ...
        if block == Block::Input {
            ...
            if let Some((raw_key, raw_value)) = line.split_once('=') {
                ...
                let value = raw_value.trim();
                if key == "follow_mouse" {
                    values.insert("focus_follow_mouse".to_owned(), value.to_owned());
                }
            }
            continue;
        }
        ...
        if line.starts_with('$') {
            if let Some((raw_name, raw_value)) = line.split_once('=') {
                let name = raw_name.trim().trim_start_matches('$').to_ascii_uppercase();
                let value = raw_value.trim().to_owned();
                if name == "MOD" {
                    values.insert("modkey".to_owned(), value.clone());
                }
        ...
        if let Some(rhs) = assignment_rhs(line, "bind") {
            parse_hyprland_bind(rhs, &variables, &mut values, &mut bind_index)?;
            continue;
        }

        if let Some(rhs) = assignment_rhs(line, "exec-once") {
            values.insert(
                format!("autostart.{autostart_index}"),
                strip_surrounding_quotes(rhs),
            );
            autostart_index += 1;
            continue;
        }
        ...
        if let Some((raw_key, raw_value)) = line.split_once('=') {
            let key = raw_key.trim().to_ascii_lowercase();
            let value = strip_surrounding_quotes(raw_value.trim());
            match key.as_str() {
                "modkey" | "main_key" => {
                    values.insert("modkey".to_owned(), value);
                }
                "terminal" => {
                    values.insert("terminal".to_owned(), value);
                }
                "launcher" => {
                    values.insert("launcher".to_owned(), value);
                }
                "border_size" => {
                    values.insert("border_size".to_owned(), value);
                }
                "gap_size" => {
                    values.insert("gap_size".to_owned(), value);
                }
                _ => {}
            }
        }
...
fn parse_hyprland_bind(
    rhs: &str,
    variables: &HashMap<String, String>,
    values: &mut HashMap<String, String>,
    bind_index: &mut usize,
) -> Result<(), CompositorError> {
    let fields = rhs.split(',').map(str::trim).collect::<Vec<_>>();
    if fields.len() < 3 {
        return Ok(());
    }

    let mods_raw = fields[0];
    let key_raw = fields[1];
    let action_raw = fields[2].to_ascii_lowercase();
    let args_raw = fields.get(3..).unwrap_or(&[]).join(",").trim().to_owned();

    if key_raw.to_ascii_lowercase().starts_with("mouse:") {
        return Ok(());
    }

    let key_token = normalize_key_token(&resolve_hypr_value(key_raw, variables));
    let mut combo_parts = Vec::new();
    for raw in mods_raw.split_whitespace() {
        let resolved = resolve_hypr_value(raw, variables);
        if resolved.is_empty() {
            continue;
        }
        combo_parts.push(normalize_hypr_modifier(&resolved));
    }
    combo_parts.push(key_token);
    let combo = combo_parts.join("+");

    let translated = match action_raw.as_str() {
        "exec" => {
            if args_raw.is_empty() {
                None
            } else {
                Some(format!("{combo} exec {args_raw}"))
            }
        }
        "killactive" => Some(format!("{combo} close_window")),
        "fullscreen" => Some(format!("{combo} fullscreen")),
        "quit" | "exit" => Some(format!("{combo} quit")),
        "reload" => Some(format!("{combo} reload_config")),
        "workspace" => Some(format!("{combo} workspace {}", args_raw.trim())),
        "movetoworkspace" => Some(format!("{combo} movetoworkspace {}", args_raw.trim())),
        "movefocus" => {
            let dir = args_raw.trim().to_ascii_lowercase();
            match dir.as_str() {
                "l" | "u" => Some(format!("{combo} focus_prev")),
                "r" | "d" => Some(format!("{combo} focus_next")),
                _ => None,
            }
        }
        unsupported => {
            tracing::warn!("ignoring unsupported hypr bind action: {unsupported}");
            None
        }
    };

    if let Some(line) = translated {
        values.insert(format!("keybind.{bind_index}"), line);
        *bind_index += 1;
    }
...
fn normalize_hypr_modifier(token: &str) -> String {
    match token.trim().to_ascii_lowercase().as_str() {
        "mod4" | "super" | "logo" | "win" | "windows" => "Super".to_owned(),
        "mod1" | "alt" => "Alt".to_owned(),
...
fn resolve_hypr_value(raw: &str, variables: &HashMap<String, String>) -> String {
    let value = raw.trim();
    if let Some(var_name) = value.strip_prefix('$') {
        variables
            .get(&var_name.to_ascii_uppercase())
            .cloned()
...
fn is_assignment_for(line: &str, key: &str) -> bool {
    assignment_rhs(line, key).is_some()
}

...
use smithay::{
    ...
    input::pointer::{GrabStartData as PointerGrabStartData, PointerGrab},
    reexports::{
...
impl PointerGrab<Raven> for ResizeSurfaceGrab {
    fn motion(
        &mut self,
        data: &mut Raven,
        handle: &mut smithay::input::pointer::PointerInnerHandle<'_, Raven>,
        _focus: Option<(
            <Raven as smithay::input::SeatHandler>::PointerFocus,
            Point<f64, Logical>,
        )>,
            ...
            .and_then(|(edges, initial_rect)| {
                ...
                edges.intersects(ResizeEdge::TOP_LEFT).then(|| {
                    let new_x = edges
                        ...
                        .then_some(initial_rect.loc.x + (initial_rect.size.w - gemoetry.size.w));

                    let new_y = edges
                        .intersects(ResizeEdge::TOP)
                        .then_some(initial_rect.loc.y + (initial_rect.size.h - gemoetry.size.h));

...
pub struct VBlankThrottle {
    event_loop: LoopHandle<'static, Raven>,
...
impl VBlankThrottle {
    pub fn new(event_loop: LoopHandle<'static, Raven>, output_name: String) -> Self {
        Self {
            event_loop,
    ...
    pub fn throttle(
        ...
        timestamp: Duration,
        mut call_vblank: impl FnMut(&mut Raven) + 'static,
    ) -> bool {
                ...
                if !self.printed_warning {
                    ...
                    tracing::warn!(
                        "output {} running faster than expected, throttling vblanks: \
                         expected refresh {:?}, got vblank after {:?}",
                        self.output_name,
                        refresh,
                        passed
                    );
